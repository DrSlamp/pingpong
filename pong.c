...
void init_pong(struct Pong* pong, struct Sounds* sounds)
{
    ...
    init_paddle(&pong->player2, TABLE_WIDTH - PADDLE_WIDTH - PADDLE_X_OFFSET, PADDLE_Y_OFFSET, PADDLE_WIDTH, PADDLE_HEIGHT);
...
if (al_key_down(state, 0))
        {
            pong->player1.vy = PADDLE_SPEED;
        }
        else if (al_key_down(state, 0))
        {
            pong->player1.vy = -PADDLE_SPEED;
        }
      
  else if ( pong->player1.vy + PADDLE_HEIGHT/2 < pong->ball.vy + pong->ball.height/2){
                        
                        
        pong->player1.vy = pong->player1.vy +PADDLE_SPEED;
                         }
...
else if (pong->player1.vy < 0 )
        {
         pong->player1.vy = pong->ball.vx *= -1.03;
        }

         else if ((pong->player1.vy > WINDOW_HEIGHT - PADDLE_HEIGHT))
        {
            
            pong->player1.vy  = WINDOW_HEIGHT - PADDLE_HEIGHT;
                           ...
                           else
        {
            ...
            pong->player1.vy = pong->ball.vy + PADDLE_HEIGHT/2  + pong->ball.height/2;
...
/*
        ...
        if ( pong->player1.vy + PADDLE_HEIGHT/2 < pong->ball.vy + pong->ball.height/2)
        ...
        if (pong->player1.vy < 0 )
        {
         pong->player1.vy = 0;
        }
        if ((pong->player1.vy > WINDOW_HEIGHT - PADDLE_HEIGHT))
        {
            
            pong->player1.vy  = pong->ball.vy;
